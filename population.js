...
// I used this code almost completely from Daniel Shiffman. 
//changed faces in flowers.
...
// this hasn't changed very much from example to example
  ...
  function Population(m, num) {
    ...
    for (var i = 0; i < num; i++) {
    this.population[i] = new Flower(new DNA(),100+i*200, 200);
  ...
  this.selection = function() {
    ...
    for (var i = 0; i < this.population.length; i++) {
      ...
      var n = floor(fitnessNormal * 15);  // multiplier is low, because otherwise there are too many flowers to draw
      for (var j = 0; j < n; j++) {
  ...
  this.reproduction = function() {
    ...
    for (var i = 0; i < this.population.length; i++) {
      ...
      this.population[i] = new Flower(child,100+i*200, 200);
    }
    this.generations++;
  }
